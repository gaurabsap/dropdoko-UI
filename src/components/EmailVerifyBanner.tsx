...
export default function EmailVerifyBanner() {
  ...
  useEffect(() => {
    // ✅ Conditions when banner should show:
    // 1. User is logged in
    // 2. Email is NOT verified
    // 3. NOT on the OTP verify page
    if (user && !isEmailVerified && pathname !== "/customer/otp-verify") {
      setVisible(true);
    } else {
      setVisible(false);
    }
  }, [user, isEmailVerified, pathname]);
  ...
  const handleVerifyNow = async () => {
    ...
    try {
      ...
      if (res?.data?.success) {
        ...
        setVisible(false); // hide banner before redirecting
  ...
  const handleDismiss = () => {
    setVisible(false); // only hides temporarily, will return on refresh
  };

  return (
    <div className="w-full bg-gradient-to-r from-amber-500 to-orange-600 text-white px-4 py-3.5 flex items-center justify-between shadow-lg z-50 animate-fade-in-down">
